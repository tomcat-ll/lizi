    ...
    stages {
        ...
        stage("发布应用") {
            steps {
                script {
                    echo "开始发布"
                    sh "curl --location --request POST ‘http://192.168.5.101:80/job/${env.JOB_NAME}/buildWithParameters‘                         --header ‘Authorization: ${env.ecarx_jenkins_auth}‘                         --form ‘env=${env.env}‘                         --form ‘branchname=origin/${env.BRANCH_NAME}‘"
                }
            }
        }
    }
